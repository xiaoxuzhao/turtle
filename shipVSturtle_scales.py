# -*- coding: utf-8 -*-
"""
Created on Thu Nov  2 15:29:13 2017
...
for i in range(len(INDX)):
    ...
    ax.plot(shiptemp[INDX[i]],shipdepth[INDX[i]],color='red' ,linewidth=2,label='ship')
    diff_turVSship=[]
    ...
    ax.plot(obstemp[t_index[0]],obsDepth[t_index[0]],color='black' ,linewidth=1,label='turtle id: '+str(turtle_id[t_index[0]]))
    ...
    for j in range(1,len(t_index)):
        if s==turtle_id[t_index[j]]:
            ...
            ax.plot(obstemp[t_index[j]],obsDepth[t_index[j]],color=c,linewidth=1,label='turtle id: '+str(s))
    ...
    ax.set_title('ship vs turtle '+'('+str(shiptime[INDX[i]].date())+')',fontsize=14)

    plt.legend(loc='upper left')
    plt.savefig('ship vs turtle  '+str(INDX[i]))        
    plt.show()
