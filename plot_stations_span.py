...
###############
dfh=read_csv(input_dir+'p_cluster_stations_60.0_bins.csv')
dfh=dfh.sort("Span")
dfh.to_csv(input_dir+'sort_p_cluster_stations_60.0_bins.csv')
df=read_csv(input_dir+'sort_p_cluster_stations_60.0_bins.csv')
span=df["Span"]
lat=df["Latitude"]
lon=df["Longitude"]
...
plt.title("the stations total span",fontsize=13)
plt.ylabel('Span')
plt.savefig(save_dir+"All_stations_span.png")
plt.show
fig = plt.figure()
a=fig.add_subplot(1,1,1)
...
plt.savefig(save_dir+'plot_stations(>1 year)',dpi=200) 
plt.show()
