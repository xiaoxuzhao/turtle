...
for i in [0]:        #two seasons together plot.
    ...
    '''
    ax1 = fig.add_subplot(121)             #plot model temperature
    ...
    cbar=plt.colorbar(CS,ticks=np.arange(int(obstemp_i.min())-1,int(obstemp_i.max())+1,2),fraction=0.046, pad=0.04)
    cbar.ax.tick_params(labelsize=6)
    #cbar.ax.set_ylabel('Temperature($^\circ$C)', fontsize=8)
    #plt.scatter(np.array(obsLon),np.array(obsLat), marker='o', c='b', s=5, zorder=10)
    ax1.set_title(season[i]+' mean',fontsize=10)
    ax1.annotate('100m depth',xy=(-75.289,35.0395),xytext=(-75.0034,34.9842),arrowprops=dict(facecolor='black'),fontsize=6)  
    ...
    '''
    ax3 = fig.add_subplot(122)                #plot model temperature
    ...
    cbar.ax.set_ylabel('Temperature($^\circ$C)', fontsize=8)
    #plt.scatter(np.array(obsLon),np.array(obsLat), marker='o', c='b', s=5, zorder=10)
    ax3.set_title(season[i+1]+' mean',fontsize=10)
    ax3.annotate('100m depth',xy=(-75.289,35.0395),xytext=(-75.0034,34.9842),arrowprops=dict(facecolor='black'),fontsize=6)  
    plt.savefig(season[i]+season[i+1]+'contour.png',dpi=200)
plt.show()