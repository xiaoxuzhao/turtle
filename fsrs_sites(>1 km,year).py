...
##########################
#HARDCODES
input_dir='/home/zdong/xiaoxu/FSRS/all_files/'#Xiaoxu's machine
...
def haversine(loni, lati, lons, lats):
       ...
       return d
# read one data files
dfh=read_csv(input_dir+'CTSeventList_4589.txt',skiprows=[0])  # extract all FSRS deployment sites   
dfh=dfh[dfh['Cruise'].str.contains('603|604|605')] #extracts only FSRS codes        
dfh.to_csv(input_dir+'getfsrs.csv')
#dfh=read_csv('/net/data5/jmanning/cts/getfsrs.csv')
dfh=read_csv(input_dir+'getfsrs.csv') #saved and opened the file in order to get fresh index
...
for i in dfh.index:
        if indexs[i]==([5000]):
             dfh.drop(i, inplace=True)        
print dfh
dfh.to_csv(input_dir+'fsrs_sites(>1 km,year).csv')